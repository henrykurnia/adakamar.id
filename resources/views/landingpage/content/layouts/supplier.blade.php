...
@section('content')
    ...
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-[#1B4EF5] sm:text-2xl dark:text-[#3874FF]">Daftar Supplier</h1>
            </div>

            <div class="items-center justify-between block sm:flex md:divide-x md:divide-[#E8D5F5] dark:divide-gray-700">
                <div class="flex items-center mb-4 sm:mb-0">
                    <form class="sm:pr-3" action="{{ route('suppliers.index') }}" method="GET" id="searchForm">
                        ...
                        <div class="relative w-48 mt-1 sm:w-64 xl:w-96">
                            ...
                            <input type="text" name="keyword" id="supplier-search" value="{{ request('keyword') }}"
                    ...
                    <div class="flex items-center w-full sm:justify-end">
                        @if(request('keyword'))
                            <a href="{{ route('suppliers.index') }}"
                                class="text-sm text-[#1B4EF5] hover:text-[#3874FF] dark:text-[#3874FF] dark:hover:text-[#5996FF] ml-2">
                                    ...
                                    <td class="p-4 text-xs text-gray-500 dark:text-gray-400 max-w-[150px]">
                                        <div class="relative inline-block w-full">
                                            <span class="block truncate cursor-help description-text"
                                                data-fulltext="{{ $supplier->address ?? '-' }}">
                                                {{ $supplier->address ?? '-' }}
                                            </span>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="5" class="text-center p-8 text-gray-500 dark:text-gray-400">
                                        ...
                                        <p class="text-sm font-medium">
                                            @if(request('keyword'))
                                                Tidak ada supplier dengan nama "{{ request('keyword') }}"
                                            @else
        ...
        <div class="p-4 bg-white border-t border-[#E8D5F5] dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Menampilkan
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $suppliers->firstItem() ?? 0 }}</span>
                    - <span class="font-semibold text-gray-900 dark:text-white">{{ $suppliers->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-900 dark:text-white">{{ $suppliers->total() }}</span>
                    data
                    @if(request('keyword'))
