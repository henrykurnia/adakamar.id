        ...
        <div class="w-full mb-1">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                ...
                <div class="flex items-center w-full sm:w-auto flex-wrap gap-2">
                    <form class="flex-1 sm:flex-none" action="{{ route('admin.stock-opnames.index') }}" method="GET"
                        id="searchForm">
                                ...
                                <tr class="hover:bg-[#F5F0FF] dark:hover:bg-gray-700 transition-colors duration-200">
                                    ...
                                    <td class="p-4">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <!-- Tombol Hapus -->
                                            <form action="{{ route('admin.stock-opnames.destroy', $opname->id) }}" method="POST"
...
<script>
    ...
    function openEditModal(opnameId) {
        // Fetch data opname menggunakan route admin
        fetch(`{{ route('admin.stock-opnames.edit', ':id') }}`.replace(':id', opnameId))
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        form.action = `{{ route('admin.stock-opnames.update', ':id') }}`.replace(':id', opnameId);
                        form.submit();
