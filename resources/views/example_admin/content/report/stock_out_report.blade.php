...
@section('content')
    ...
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-[#1B4EF5] sm:text-2xl dark:text-[#3874FF]">Laporan Barang Keluar</h1>
            </div>

            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                <!-- Bagian Kiri: Filter Form -->
                <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    <form class="flex flex-wrap items-center gap-2 w-full sm:w-auto"
                        action="{{ route('admin.reports.stock-out') }}" method="GET" id="filterForm">
                        <!-- Search Keyword -->
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center p-6 text-xs text-gray-500 dark:text-gray-400">
                                        @if(request('keyword') || request('supplier') || request('date') || request('month'))
                                            Tidak ada transaksi barang keluar dengan filter yang dipilih
                                        @else
    ...
    <form id="exportForm" action="{{ route('admin.reports.stock-out.export') }}" method="GET" target="_blank">
        ...
        if (exportButton && exportForm) {
            exportButton.addEventListener('click', function (e) {
                ...
                Swal.fire({
                    ...
                    text: 'Anda akan mengekspor data laporan barang keluar ke Excel. Lanjutkan?',
                    icon: 'question',
