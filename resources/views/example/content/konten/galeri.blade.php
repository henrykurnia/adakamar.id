...
@section('content')
    ...
    @include('example.layouts.partials.konten.modal_add_gallery')
    @include('example.layouts.partials.konten.modal_edit_gallery')
    ...
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                <h1 class="text-xl font-semibold text-[#E60000] sm:text-2xl dark:text-[#FF6B6B]">Daftar Galeri</h1>
            </div>

            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                <!-- Bagian Kiri: Search -->
                <div class="flex items-center w-full sm:w-auto flex-wrap gap-2">
                    <form class="flex-1 sm:flex-none" action="{{ route('galleries.index') }}" method="GET"
                        id="searchForm">
                ...
                <div class="flex items-center w-full sm:w-auto sm:justify-end">
                    <button onclick="openAddGalleryModal()"
                        ...
                        <tbody class="bg-white divide-y divide-[#FFD4D4] dark:bg-gray-800 dark:divide-gray-700"
                            ...
                            @forelse($galleries as $index => $gallery)
                                <tr data-id="{{ $gallery->id }}">
                                    ...
                                    <td class="p-4 text-xs text-gray-900 dark:text-white font-medium max-w-[200px] truncate">
                                        {{ $gallery->title }}
                                    </td>
                                    ...
                                    <td class="p-4 text-xs">
                                        <div class="flex items-center gap-1.5">
                                            <button onclick="openEditGalleryModal({{ $gallery->id }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center p-6 text-gray-500 dark:text-gray-400">
                                        @if (request('keyword'))
                                            Tidak ada galeri dengan judul "{{ request('keyword') }}"
                                        @else
        ...
        <div class="p-4 bg-white border-t border-[#FFD4D4] dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Menampilkan
                    <span class="font-semibold text-gray-900 dark:text-white">{{ $galleries->firstItem() ?? 0 }}</span>
                    - <span class="font-semibold text-gray-900 dark:text-white">{{ $galleries->lastItem() ?? 0 }}</span>
                    dari <span class="font-semibold text-gray-900 dark:text-white">{{ $galleries->total() }}</span>
                    data
                    @if (request('keyword'))
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                const name = this.dataset.name || 'gambar ini';

                Swal.fire({
                    title: 'Apakah Anda yakin?',
                    text: `Anda akan menghapus galeri "${name}" secara permanen!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        form.method = 'POST';
                        form.action = '{{ route('galleries.destroy', '') }}/' + id;
                        form.innerHTML = `
                            @csrf
                            @method('DELETE')
                        `;
        ...
        console.log('Halaman galeri siap digunakan.');
    });
</script>
